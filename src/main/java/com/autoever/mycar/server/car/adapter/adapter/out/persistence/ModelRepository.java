...
public interface ModelRepository extends JpaRepository<Model, Long>{
    ...
    @Query(value = "select distinct m.id modelId, t.name trimName, m.price, m.basic_info basicInfo, t.code trimCode, c.code carCode " +
            ...
            "on m.id = d.model_id " +
            "join car c, trim t " +
            "where c.code = t.car_code and t.code = m.trim_code and t.code = :trimCode and e.tool_id = :engineId", nativeQuery = true)
    ...
    @Query(value = "select distinct m.id modelId, t.name trimName, m.price, m.basic_info basicInfo, t.code trimCode, c.code carCode " +
            ...
            "join car c, trim t " +
            "where c.code = t.car_code and t.code = m.trim_code and " +
            "t.code = :trimCode and e.tool_id = :engineId and g.tool_id = :gearboxId and d.tool_id = :drivingId",
            nativeQuery = true)
    Optional<TrimResDto> findByTrimCodeAndTooltipId(String trimCode, Long engineId, Long gearboxId, Long drivingId);
